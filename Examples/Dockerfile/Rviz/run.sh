...
docker build -t ros-nvidia-6.5-rviz ./ && nvidia-docker run -ti --rm --name="rviz-kinetic" --hostname="rviz" -e DISPLAY=$DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix -v /dev/dri/:/dev/dri/ --env="QT_X11_NO_MITSHM=1"   ros-nvidia-6.5-rviz   /bin/bash -c 'sleep 5; wget http://${ROS_DNS_SERVER}/register.sh;  bash register.sh;   sleep 5; su newuser -c "source /opt/ros/kinetic/setup.bash; rosrun rviz rviz "' 
